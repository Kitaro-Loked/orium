...
export class ChutesAdapter extends ModelAdapter {
  ...
  async complete(request: CompletionRequest): Promise<CompletionResponse> {
    const res = await fetch(`${this.baseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        model: request.model || 'chutesai/Llama-3.3-70B-Instruct',
    ...
    if (!res.ok) {
      throw new Error(`Chutes error: ${res.status} ${await res.text()}`);
    }
